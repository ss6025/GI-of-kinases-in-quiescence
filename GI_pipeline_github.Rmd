...
##save table S3: filtered and vst transfromed counts table. Original datat for Fig1C & Fig2A
write.csv(VST_normalized_filtered, "./GI_quiescence/sum/tables/TableS3.csv")
...
#TableS4: Linear regression modeling of mutant fitness over prolonged starvation for each replicate. Original data for Fig 1B (.csv).
write.csv(VST_lm_prolonged_rep_wtNormalized, "./GI_quiescence/sum/tables/TableS4.csv")
```
...
#save the file 
#TableS5: Linear regression modeling of mutant fitness over prolonged starvation across all replicate. Original data for Fig 1C (.csv).
...
###heatmaps & PCAs
```{r}
VST_lm_prolonged_rep_wtNormalized <- read_csv("./GI_quiescence/sum/tables/TableS4.csv")[,-1]
...
```{r, echo =F}
VST_normalized_filtered <- read_csv("./GI_quiescence/sum/tables/TableS3.csv")[,-1]
VST_lm_prolonged_wtNormalized <- read_csv("./GI_quiescence/sum/tables/TableS5.csv")[,-1]
...
##################################save files 
#TableS6: fitness and survival rate modeling for cells in different growth stages. Original data for Fig2B & FigS2A.
write.csv(VST_lm_stages_wtNormalized, "./GI_quiescence/sum/tables/TableS6.csv")
```

###Fitness and survival phenotypic difference quantification by ANCOVA
```{r}
VST_normalized <- read_csv("./GI_quiescence/sum/tables/TableS3.csv")
###########################################################################################
...
###########################################################################################
#save for TableS7: Phenotypic difference between quiescence and proliferation quantified by ANCOVA.
write.csv(fitness_diff_QC_HO_wtNormalized, "./GI_quiescence/sum/tables/TableS7.csv")
```
...
VST_lm_stages<-read_csv("./GI_quiescence/sum/tables/TableS6.csv")[,-1]
VST_lm_stages<-left_join(VST_lm_stages, YtoGene, by = "Strain")

fitness_diff_QC_HO <- read_csv("./GI_quiescence/sum/tables/TableS7.csv")[,-1]
...
#TableS8: Fitness/survival and phenotypic difference in one same table. Original table for FigS2C.
write_csv(fitness_G_S_diff_HO_byRow, "./GI_quiescence/sum/tables/TableS8.csv")
...
#Table9: Quiescent specific genes across different conditions. Original data for Fig2C
write_csv(QC_required_fig2, "./GI_quiescence/sum/tables/TableS9.csv")
...
#TableS10: Original data for FigS2D
write_csv(QS, "./GI_quiescence/sum/tables/TableS10.csv")
```

##FigureS2E
```{r}
#select the essential part for heatmap & save in a dataframe
core_set <- read_csv("./GI_quiescence/sum/tables/TableS10.csv")
...
# A list for results holding
  ...
  gseGOtable %>% write_csv(path=paste0("./GI_quiescence/sum/GO_term/TableS11_gseGO_changed_",i,".csv"), append=T, col_names = T)
}
```
...
BP <- read_csv(paste0(dir,"TableS11_gseGO_changed_BP.csv"))
...
#save file
#TableS11: GSEA for QS genes in three different conditions. Original data for Fig2C
write_csv(BP_filtered, "./GI_quiescence/sum/tables/TableS11.csv")
...
VST_filtered_stages <- read_csv("./GI_quiescence/sum/tables/TableS6.csv")[,-1]
...
#save files 
#TableS12: Genetic interaction quantification by ANCOVA using different phenotypic readout for proliferation and quiescence. 
write.csv(GI_VST_all_wtNormed, "./GI_quiescence/sum/tables/TableS12.csv")
```
...
GI_VST_all_wtNormed <- read_csv ("./GI_quiescence/sum/tables/TableS12.csv")[,-1]
VST_lm_stage_wtNormed <- read_csv("./GI_quiescence/sum/tables/TableS6.csv")[,-1]
...
#TableS13: genetic interaction similarity for all kinases during proliferation. Original data for Fig4A
write.csv(master_GI_pro, "./GI_quiescence/sum/tables/TableS13.csv", row.names = FALSE)
...
#TableS14: genetic interaction similarity for all kinases during proliferation. Original data for Fig4B
write.csv(master_GI_qui, "./GI_quiescence/sum/tables/TableS14.csv", row.names = FALSE)
...
#save TableS15: Functional annotation of gene clusters that are positively interacting with rim15 in quiescent cells. Original data for Fig6A.
...
#save TableS16:Functional annotation of gene clusters that are negatively interacting with rim15 in quiescent cells. Original data for Fig6B.

write_csv(formula_res_all_neg_min, "./GI_quiescence/sum/tables/TableS16.csv")
...
GI_VST_all_wtNormed<-read_csv("./GI_quiescence/sum/tables/TableS12.csv") %>%
  dplyr::rename(Systematic = Strain)
...
#save TableS17: gene clusters that a negatively interacting with tor1 in quiescent cells. Original data for FigS6B
write_csv(formula_res_pos_min, "./GI_quiescence/sum/tables/TableS17.csv")
...
#save TableS18: gene clusters that a negatively interacting with tor1 in quiescent cells. Original data for FigS6B
write_csv(formula_res_neg_min, "./GI_quiescence/sum/tables/TableS18.csv")
...
path = "./GI_quiescence/sum/plot/"
GI_VST_all_wtNormed<-read_csv("./GI_quiescence/sum/GIS analysis/TableS12.csv")[,-1]
