...
YtoGene <- read.table("~/Desktop/GI_quiescence/DESeq2 normalized data/YORFtoGENE_2018")
colnames(YtoGene)<-c("Strain","Common")
#this file can be read in later for gene name and systematic name converting

#set working directory
datadir <- "~/Desktop/GI_quiescence/Raw data/"

#read in index files
index_c <- read_csv(paste0(datadir, "C-lim-4mutants/seqdata/GeneInter_C_lim_offtime.csv"))

index_n <- read_csv(paste0(datadir, "N-lim-4mutants/seqdata_rerun/GeneInter_N_lim.csv"))

index_n_new <- read_csv(paste0(datadir, "N-lim-3mutants/seqdata/GeneInter_N_lim.csv"))

index_p <- read_csv(paste0(datadir, "P-lim-4mutants/seqdata/GeneInter_P_lim.csv"))

#read in counts files out of Barnone with the pretested mismatches (2MM)
ct <-  read.table(paste0(datadir,"C-lim-4mutants/seqdata/C_lim.barnone.2MM.counts.txt"), sep="\t", header = TRUE)
nt <-  read.table(paste0(datadir,"N-lim-4mutants/seqdata_rerun/N_lim.barnone.2MM.counts.txt"), sep="\t", header = TRUE)
nt_new <-  read.table(paste0(datadir,"N-lim-3mutants/seqdata/N_lim.barnone.2MM.counts.txt"), sep="\t", header = TRUE)
pt <-  read.table(paste0(datadir,"P-lim-4mutants/seqdata/P_lim.barnone.2MM.counts.txt"), sep="\t", header = TRUE)
```
...
write.csv(filtered_libs_all, file = "~/Desktop/GI_quiescence/sum/tables/filtered_libs_all.csv")
```

##Counts merging for passed filter libraries (Up Tag + Down Tag)
```{r}
#read in the output of filtered libraries
filtered_libs_all<-read.csv("~/Desktop/GI_quiescence/sum/tables/filtered_libs_all.csv")[,-1]
...
write.csv(up_filtered, "~/Desktop/GI_quiescence/sum/tables/up_filtered.csv")
write.csv(dn_filtered, "~/Desktop/GI_quiescence/sum/tables/dn_filtered.csv")
write.csv(add_filter, "~/Desktop/GI_quiescence/sum/tables/merged_filtered.csv")
```

##Counts normalization and variable stabilization transformation
```{r}
add_filter<-read.csv("~/Desktop/GI_quiescence/sum/tables/merged_filtered.csv")[,-1]
...
write_csv(countdata, "~/Desktop/GI_quiescence/sum/tables/countdata.csv")

#read in the colData after adding the new rows for N-lim ho, rim15, pho85 rerun on Mar 2019
index_file <- read_csv("~/Desktop/GI_quiescence/Raw data/index template/GeneInter.csv")
...
#prefiltering
#dds <- ddsMat[rowSums(counts(ddsMat)) > 3000, ]
...
dds <- estimateSizeFactors(dds)
  #ds <- estimateDispersions(dds)
  #plotDispEsts(ds)
#normalize the data 
  #deseq_normed<-counts(dds, normalized=TRUE)
...
write.csv(tmp_vst, "~/Desktop/GI_quiescence/sum/tables/vst_melt_local_filtered_reps.csv")
```
...
```{r, include = F}
VST_normalized <- read_csv("~/Desktop/GI_quiescence/sum/tables/vst_melt_local_filtered_reps.csv")[,-1]
...
write.csv(VST_normalized_filtered, "~/Desktop/GI_quiescence/sum/tables/filtered_reps_wtNormalized_new.csv")

#save the table into RData for shiny app: http://shiny.bio.nyu.edu/ss6025/shiny_Genetic_Interaction/
save(VST_normalized_filtered, file = "~/Desktop/GI_quiescence/shiny_Genetic_Interaction/appdata/VST_normalized_filtered_shiny_new.RData")
...
write.csv(VST_lm_prolonged_rep_wtNormalized, "~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_prolonged_filtered_rep_wtNormalized_new.csv")
```

###Prolong fitness quantification by linear regression modeling for all replicates
```{r, include = F}
#save the files with relabed replicates 
VST_normalized_filtered <- read_csv("~/Desktop/GI_quiescence/sum/tables/filtered_reps_wtNormalized_new.csv")[,-1]
...
#save the file
write.csv(VST_lm_prolonged_wtNormalized, "~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_prolonged_filtered_wtNormalized_new.csv")
```

###Looking for know synthetic lethality mutant in our data
```{r, include = F}
#read in published genetic interaction data for rim15, tor1 and pho85
rim15<-read.delim("~/Desktop/GI_quiescence/sum/known SGA data from BIOGRID/BIOGRID-GENE-31114-3.5.174.tab2.txt")

tor1<-read.delim("~/Desktop/GI_quiescence/sum/known SGA data from BIOGRID/BIOGRID-GENE-33823-3.5.174.tab2 2.txt")

pho85<-read.delim("~/Desktop/GI_quiescence/sum/known SGA data from BIOGRID/BIOGRID-GENE-36147-3.5.174.tab2.txt")

#maintain the info with genetic interactions from Costanzo M (2010)
rim15_gi<-rim15 %>%
  filter(Experimental.System == "Synthetic Lethality" & Throughput == "High Throughput")
...
#looking for those genes in our dataset
VST_normalized<-read_csv("~/Desktop/GI_quiescence/sum/tables/filtered_reps_wtNormalized_new.csv")[,-1]
rim15_T0 <- VST_normalized %>% filter(LibraryName == "RIM15" & SamplingTime == "0")
#overlaps
print(paste0("Known synthetic lethality genes with rim15: ", as.factor(intersect(rim15_gi$Official.Symbol.Interactor.A, rim15_T0$Common))))

tor1_T0 <- VST_normalized %>% filter(LibraryName == "TOR1" & SamplingTime == "0")
#overlaps
print(paste0("Known synthetic lethality genes with tor1: ",intersect(tor1_gi$Official.Symbol.Interactor.B, tor1_T0$Common)))

pho85_T0 <- VST_normalized %>% filter(LibraryName == "PHO85" & SamplingTime == "0")
#overlaps
print(paste("Known synthetic lethality genes with pho85: ", c(intersect(pho85_gi$Official.Symbol.Interactor.A, pho85_T0$Common),
intersect(pho85_gi$Official.Symbol.Interactor.B, pho85_T0$Common))))
#we do see some genes that suppose to be absent accroding to Tong AH (2004), Aronova S (2007), Daniel JA (2006), Scarcelli JJ (2008), Lesage G (2004), Krause SA (2012), van Pel DM (2013).
```

##Figure 1B & Appendix figures 
###heatmaps & PCAs
```{r}
VST_lm_prolonged_rep_wtNormalized <- read_csv("~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_prolonged_filtered_rep_wtNormalized_new.csv")[,-1]
...
pdf("~/Desktop/GI_quiescence/sum/plot/Appendix_Figure1.pdf",width=7,height=5)
...
#overall
#save filtered countdata
countdata <- read_csv("~/Desktop/GI_quiescence/sum/tables/countdata.csv")
...
pdf("~/Desktop/GI_quiescence/sum/plot/Figure1B.pdf")
#all
pheatmap(as.matrix(fit_all[,2:length(fit_all[,])]), annotation_col = annotation[,2:3])#, show_colnames = F)
dev.off()

pdf("~/Desktop/GI_quiescence/sum/plot/Appendix_Figure2.pdf")
#cut by nutrient
#carbon
...
```{r, echo =F}
VST_normalized_filtered <- read_csv("~/Desktop/GI_quiescence/sum/tables/filtered_reps_wtNormalized_new.csv")[,-1]
VST_lm_prolonged_wtNormalized <- read_csv("~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_prolonged_filtered_wtNormalized_new.csv")[,-1]
...
ggsave("~/Desktop/GI_quiescence/sum/plot/individual gene/ATG3/ATG3_fitness_lineplot.pdf", width = 7, height = 5)
...
#extract the fitness for TOR1 in ho and barplot it
 ...
 p <- ggplot(df, aes(x=nutrient, y=estimate, fill=nutrient)) +
  ...
  ggsave(plot=p, file = paste0("~/Desktop/GI_quiescence/sum/plot/individual gene/",gene,"_fitness_dotplot.pdf"),width=7,height=5)
}
...
write_csv(VST_normalized_filtered, "~/Desktop/GI_quiescence/sum/tables/filtered_reps_updated_wtNormalized_new.csv")[,-1]
...
##################################save files 
write.csv(VST_lm_stages_wtNormalized, "~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_stages_wtNormalized_new.csv")
```

###Fitness and survival phenotypic difference quantification by ANCOVA
```{r}
VST_normalized <- read_csv("~/Desktop/GI_quiescence/sum/tables/filtered_reps_updated_wtNormalized_new.csv")
###########################################################################################
...
grow_df <- VST_normalized %>% dplyr::select(-SampleNum) %>% filter(GrowStage == "G") %>% drop_na()
starved_df <- VST_normalized %>% dplyr::select(-SampleNum) %>% filter(GrowStage == "S") %>% drop_na()
...
fitness_diff_QC_HO_wtNormalized <- gs_df %>%
      ...
      filter(term == "scaled_time:GrowStageS") %>%
      ungroup() %>% filter(term == "scaled_time:GrowStageS", !is.na(p.value)) %>%
      mutate(q.value = qvalue(p.value, fdr.level=0.05, pi0.method="bootstrap")$qvalues)

###########################################################################################
###########################ANCOVA fitness difference modeling end##########################
###########################################################################################
write.csv(fitness_diff_QC_HO_wtNormalized, "~/Desktop/GI_quiescence/sum/tables/fitness(lm)_diff(ANCOVA)_QC_HO_wtNormalized_new.csv")
```
...
ggsave("~/Desktop/GI_quiescence/sum/plot/individual gene/ATG3/ATG3_fitness_lineplot_stages.pdf", width = 7, height = 4)
```
...
VST_lm_stages<-read_csv("~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_stages_wtNormalized_new.csv")[,-1]
VST_lm_stages<-left_join(VST_lm_stages, YtoGene, by = "Strain")

fitness_diff_QC_HO <- read_csv("~/Desktop/GI_quiescence/sum/tables/fitness(lm)_diff(ANCOVA)_QC_HO_wtNormalized_new.csv")[,-1]

fitness_diff_QC_HO <- fitness_diff_QC_HO %>% mutate(GrowStage = "S-G")

#seperate fitness based on stages
fitness_growing_HO <- VST_lm_stages %>%
  dplyr::filter(LibraryName == "HO"  & GrowStage == "G") %>%
  dplyr::select(-c(LibraryName, term, std.error, statistic))

fitness_starved_HO <- VST_lm_stages %>%
  dplyr::filter(LibraryName == "HO"  & GrowStage == "S") %>%
  dplyr::select(-c(LibraryName, term, std.error, statistic))
...
write_csv(fitness_G_S_diff_HO_byRow, "~/Desktop/GI_quiescence/sum/tables/TableS1.csv")

#Figure 2SC
#violen plot for fitness distribution
fitness_G_S_diff_HO_byRow %>%
  #filter(GrowStage != "S-G") %>%
...
ggsave("~/Desktop/GI_quiescence/sum/plot/figS2B.pdf", width = 9, height = 7)

#ggsave("~/Desktop/Figures for paper/Main figures/Figure2/figS2B.pdf", width = 9, height = 7)
...
print(paste0("carbon:",length(carbon_qc_required$Systematic),"; nitrogen:", length(nitrogen_qc_required$Systematic), "; phosphate:", length(phosphate_qc_required$Systematic), "; carbon&nitrogen:",length(cn_qc_required), "; carbon&phosphate:", length(cp_qc_required), "; nitrogen&phosphate:",length(np_qc_required), "; carbon&nitrogen&phosphate:", length(cnp_qc_required)))

#draw a venn diagram
library(VennDiagram)

pdf("~/Desktop/GI_quiescence/sum/plot/Fig2_vendiagram_lm_wtNormed(p.adj<0.05).pdf")
grid.newpage()
...
read_tsv("~/Desktop/GI_quiescence/sum/data/sgd_go_slim_171013.txt",col_names=FALSE,
  comment="!") -> SGDGOSlim

# Full GO annotation table
read_tsv("~/Desktop/GI_quiescence/sum/data/sgd_go_full_171013.txt",col_names=FALSE,
  comment="!") -> SGDGOFull

# Go terms look up table
read_tsv("~/Desktop/GI_quiescence/sum/data/sgd_go_terms_171013.txt",col_names=FALSE,
  comment="!") %>%
...
###Looking for quiescent-specific (QS) genes
```{r include=FALSE}
#SystematicToSGDIDmapper need to be read in via a different file, GSEA_gseGO.Rmd
gene_list <- QC_required_fig2 %>% filter(Systematic %in% cnp_qc_required) %>%
...
cnp_qc_required_df<- QC_required_fig2 %>% filter(Systematic %in% cnp_qc_required) %>%
        left_join(SystematicToSGDIDmapper,by="Systematic")

cnp_qc_required_genes<-unique(cnp_qc_required_df$Systematic)

#colnames(YtoGene) <- 

QS_genes <- data.frame(cnp_qc_required_genes) %>%
  dplyr::rename(Systematic = cnp_qc_required_genes)
QS_genes <- left_join(QS_genes, YtoGene %>% rename(Systematic = Strain), by = "Systematic") %>%
  dplyr::select(Systematic, Common)
QS_genes$Common <- as.character(QS_genes$Common)
QS_genes$Common[is.na(QS_genes$Common)] <- QS_genes$Systematic[is.na(QS_genes$Common)]

write_csv(QS_genes, "~/Desktop/GI_quiescence/sum/tables/CNP_QC_Required_gens(p<0.05).csv")


#check the over representative terms,
background_list <-  fitness_G_S_diff_HO_byCol %>% 
  filter(Systematic %in% unique(fitness_G_S_diff_HO_byCol$Systematic)) %>%
  left_join(SystematicToSGDIDmapper,by="Systematic")%>%
  {clusterProfiler::bitr(.$SGDID,fromType="SGD",toType="ENTREZID", OrgDb=org.Sc.sgd.db::org.Sc.sgd.db)}

fitness_G_S_diff_HO_fig2 <- fitness_G_S_diff_HO_byCol %>%
  dplyr::rename( Change_slope = estimate, Growing_slope = estimate.g, Starved_slope = estimate.s) %>%
  dplyr::select(nutrient, Systematic, Growing_slope, Starved_slope, Change_slope)

fitness_G_S_diff_HO_fig2 <- left_join(fitness_G_S_diff_HO_fig2, YtoGene, by="Systematic")

#save files individually depend on limited nutrient and then perform term_enrichment analysis
fitness_G_S_diff_HO_fig2_c <- fitness_G_S_diff_HO_fig2 %>% filter( nutrient == "carbon")
fitness_G_S_diff_HO_fig2_n <- fitness_G_S_diff_HO_fig2 %>% filter( nutrient == "nitrogen")
fitness_G_S_diff_HO_fig2_p <- fitness_G_S_diff_HO_fig2 %>% filter( nutrient == "phosphate")

write.csv(fitness_G_S_diff_HO_fig2_c, "~/Desktop/GI_quiescence/sum/tables/fitness(lm_wtNormed)_G_S_diff_HO_fig2_c.csv", row.names = TRUE)
write.csv(fitness_G_S_diff_HO_fig2_n, "~/Desktop/GI_quiescence/sum/tables/fitness(lm_wtNormed)_G_S_diff_HO_fig2_n.csv", row.names = TRUE)
write.csv(fitness_G_S_diff_HO_fig2_p, "~/Desktop/GI_quiescence/sum/tables/fitness(lm_wtNormed)_G_S_diff_HO_fig2_p.csv", row.names = TRUE)
```

##FigureS2E
```{r}
#select the essential part for heatmap & save in a dataframe
QS_genes <- read_csv("~/Desktop/GI_quiescence/sum/tables/CNP_QC_Required_gens(p<0.05).csv")

core_set <- subset(fitness_G_S_diff_HO_byRow, Common %in% QS_genes$Common | Systematic %in% QS_genes$Common) %>%
  data.frame()

#conver the factor column: Common into character
core_set$Common <- as.character(core_set$Common)
#replace the NA values in Common by Systematic name
core_set$Common[is.na(core_set$Common)] <- core_set$Systematic[is.na(core_set$Common)]

core_set<-left_join(core_set, QS_genes, by = "Common")

common_genes <- core_set %>%
  filter(GrowStage %in% c("S","G")) %>%
...
#core
my_gene_row <- core_set %>% filter(nutrient =="carbon" & GrowStage == "G") %>% dplyr::select(Common) %>% as.data.frame()
...
library(pheatmap)
pdf("~/Desktop/GI_quiescence/sum/plot/FigS2E.pdf")
...
# A list for results holding
  ...
  gseGOtable %>% write_csv(path=paste0("~/Desktop/GI_quiescence/sum/GO_term/output_table_gseGO_newN_changed_",i,".csv"), append=T, col_names = T)
}
```

##Figure 2C
```{r}
dir<-"~/Desktop/GI_quiescence/sum/GO_term/"
BP <- read_csv(paste0(dir,"output_table_gseGO_changed_BP.csv"))
...
BP_filtered <- bind_rows(left_join(c_removed %>% dplyr::select (term_ID, Variable, uniqueness), BP %>% rename(term_ID = ID), by = c("term_ID","Variable")), 
                         left_join(n_removed %>% dplyr::select (term_ID, Variable, uniqueness), BP %>% rename(term_ID = ID), by = c("term_ID","Variable")),
                         left_join(p_removed %>% dplyr::select (term_ID, Variable, uniqueness), BP %>% rename(term_ID = ID), by = c("term_ID","Variable"))) %>%
  filter(setSize < 400 & setSize > 5)
  
write_csv(BP_filtered, "~/Desktop/GI_quiescence/sum/GO_term/gseGO_changed_BP_filtered(revigo).csv")

#open the BP_filtered file in excel and remove the redundant terms
####OR
#BP_filtered <- BP_filtered %>% filter(!term_ID %in% c("GO:0070814", "GO:0018410","GO:1901362","GO:0034660","GO:0051051","GO:1903008","GO:0006368","GO:0045005","GO:0045003","GO:0000103","GO:0098732","GO:0043603","GO:0006353","GO:0034227","GO:0030242","GO:0023052","GO:0007033","GO:0007034","GO:0016192","GO:0007323","GO:0071852","GO:0030447","GO:0098876","GO:0008219","GO:0048584","GO:0048519","GO:0035825","GO:0007034"))

#read in simplfied GOs returned by REVIGO
changed_BP_RE_simplist <- read_csv(paste0(dir,"gseGO_changed_BP_filtered(revigo_manual).csv"))
...
changed_BP_RE_simplist %>%
  ...
  theme_minimal()
#save the plot
ggsave(dpi=200, filename="~/Desktop/GI_quiescence/sum/plot/ordered_GSEA_simplest_new.pdf", useDingbats=FALSE, height = 8, width = 7)
```

##Figrue 2B
```{r,warning=F,message=F}
gseGO_changed_BP_filtered <- read_csv("~/Desktop/GI_quiescence/sum/GO_term/gseGO_changed_BP_filtered(revigo_manual).csv")
...
#fitness_G_S_diff_HO_byRow <- read_csv("~/Desktop/GI_quiescence/sum/fitness_G_S_diff_HO_byRow.csv")
...
fitness_G_S_diff_HO_byRow %>%
  ...
  geom_text_repel(data=GO_terms, aes(label=COMMON), col = "black", size=3) +
  geom_line(data=GO_terms, aes(group = COMMON, col=nutrient), alpha = 0.5, linetype = "dashed")
ggsave("~/Desktop/GI_quiescence/sum/plot/Fig2_violen_mutants_GSEA.pdf", width = 9, height = 6)
```

###FigureS2A
```{r}
#####correlatino bt nutrientional starvations under the same condition
VST_filtered_stages <- read_csv("~/Desktop/GI_quiescence/sum/tables/VST_fitness(lm)_stages_wtNormalized_new.csv")[,-1]
...
VST_filtered_stages %>% 
  ...
  geom_density(alpha = 0.1)+
  facet_grid(GrowStage~nutrient)#, scales = "free")
...
library(plyr)
pdf("~/Desktop/GI_quiescence/sum/plot/FigS2A.pdf",width=5, height=7)
# Main plot
pmain <- ggscatter(lm_wt_growstage_corr, x = "G", y = "S", alpha = 0.5, size=0.5, cor.coef.size = 0.3, col= "nutrient")+
...
xdens <- axis_canvas(pmain, axis = "x")+
  geom_density(data = lm_wt_growstage_corr, aes(x = G, fill = nutrient),
              alpha = 0.7, size = 0.2)+
  scale_color_manual(values=c("#FFA500", "#7ACE0D", "#A564F9"))+
  scale_fill_manual(values=c("#FFA500", "#7ACE0D", "#A564F9"))
...
ydens <- axis_canvas(pmain, axis = "y", coord_flip = TRUE)+
  geom_density(data = lm_wt_growstage_corr, aes(x = S, fill = nutrient),
                alpha = 0.7, size = 0.2)+
  coord_flip()+
  scale_color_manual(values=c("#FFA500", "#7ACE0D", "#A564F9"))+
  scale_fill_manual(values=c("#FFA500", "#7ACE0D", "#A564F9"))
...
VST_normalized_filtered <- read_csv("~/Desktop/GI_quiescence/sum/tables/filtered_reps_updated_wtNormalized_new.csv")
...
write.csv(GI_VST_all_wtNormed, "~/Desktop/GI_quiescence/sum/GIS analysis/anova_GI_VST_stage_wtNormed(lm).csv")
```
...
path = "~/Desktop/GI_quiescence/sum/plot/"
gene = "ATG7"
...
ggsave(paste0(path,"ATG7_ANCOVA_sample(TOR1_RIM15_PHO85).pdf"), width=12,height=5)

VST_normalized_filtered %>%
  filter(LibraryName %in% c("HO","TOR1"),Common %in% gene) %>%
  filter(SamplingTime %in% c("32","48","96","186","348")) %>%
  filter(!(Common == gene & LibraryName == gene)) %>%
  ggplot(aes(x=SamplingTime, y=Rela2WT, group=LibraryName, color=LibraryName)) +
  geom_point(aes(shape = LibraryName))+
  ...
  xlab("Hours post inoculation")+
  ylab("Relative frequency to query mutant")+
  scale_color_manual(values=c("#E5C494","#66C2A5"))+
  scale_fill_manual(values=c("#E5C494","#66C2A5"))+
  facet_grid(~nutrient, scales = "free") +
  theme(legend.position="bottom")
ggsave(paste0(path,"ATG7_ANCOVA_sample(TOR1).pdf"), width=12,height=5)

VST_normalized_filtered %>%
  filter(LibraryName %in% c("HO","RIM15"), Common %in% gene) %>%
  filter(Common %in% gene) %>%
  filter(SamplingTime %in% c("32","48","96","186","348")) %>%
  filter(!(Common == gene & LibraryName == gene)) %>%
  ggplot(aes(x=SamplingTime, y=Rela2WT, group=LibraryName, color=LibraryName)) +
  ...
  xlab("Hours post inoculation")+
  ylab("Relative frequency to query mutant")+
  scale_color_manual(values=c("#E5C494","#E78AC3"))+ #"#EDB2DC","#98EAAC","#7DB1DB","#F2C68C"
  scale_fill_manual(values=c("#E5C494","#E78AC3"))+ 
  facet_grid(~nutrient, scales = "free") +
  theme(legend.position="bottom")
ggsave(paste0(path,"ATG7_ANCOVA_sample(RIM15).pdf"), width=12,height=5)

VST_normalized_filtered %>%
  filter(LibraryName %in% c("HO","PHO85"), Common %in% gene) %>%
  filter(Common %in% gene) %>%
  filter(SamplingTime %in% c("32","48","96","186","348")) %>%
  filter(!(Common == gene & LibraryName == gene)) %>%
  ggplot(aes(x=SamplingTime, y=Rela2WT, group=LibraryName, color=LibraryName)) +
  ...
  xlab("Hours post inoculation")+
  ylab("Relative frequency to query mutant")+
  scale_color_manual(values=c("#E5C494","#8DA0CB"))+ #"#EDB2DC","#98EAAC","#7DB1DB","#F2C68C"
  scale_fill_manual(values=c("#E5C494","#8DA0CB"))+ 
  facet_grid(~nutrient, scales = "free") +
  theme(legend.position="bottom")
ggsave(paste0(path,"ATG7_ANCOVA_sample(PHO85).pdf"), width=12,height=5)
...
for (n in sort(unique(VST_normalized_filtered$nutrient))){
  for (i in unique(VST_normalized_filtered$LibraryName)){
    tmp <- 
      VST_normalized_filtered %>%
      filter(SamplingTime %in% c("32","48","96","186","348")) %>%
      filter(LibraryName == i, Strain == "YHR171W" & nutrient == n) %>%
      group_by(nutrient, LibraryName, Strain) %>%
...
GI_VST_all_wtNormed <- read_csv ("~/Desktop/GI_quiescence/sum/GIS analysis/anova_GI_VST_stage_wtNormed(lm).csv")[,-1]
VST_lm_stage_wtNormed <- read_csv("~/Desktop/GI_quiescence/sum/VST_fitness(lm)_stages_wtNormalized_new.csv")[,-1]
...
write_csv(RIM15, "~/Desktop/GI_quiescence/sum/GIS analysis/GIS_methods_comp_wtNormed_RIM15.csv")
...
ggsave("~/Desktop/GI_quiescence/sum/plot/Appendix_Figure3A.pdf" , width = 7, height = 5)
...
ggscatter(RIM15 %>% filter(nutrient == "phosphate"), x = "estimate.single", y = "estimate.double", alpha = 0.7, size=0.7, 
  ...
  theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), 
        axis.title=element_text(size=12,face="bold")) 
ggsave("~/Desktop/GI_quiescence/sum/plot/Figure3C.pdf", width = 7, height = 7)
...
ggscatter(RIM15, x = "estimate.single", y = "estimate.double", alpha = 0.7, size=0.7, 
  ...
  theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), 
        axis.title=element_text(size=12,face="bold")) 
ggsave("~/Desktop/GI_quiescence/sum/plot/FigS3B.pdf", width = 7, height = 7)
...
write_csv(TOR1, "~/Desktop/GI_quiescence/sum/GIS analysis/GIS_methods_comp_wtNormed_TOR1.csv")
...
ggscatter(TOR1, x = "ancova.gis", y = "add.gis", alpha = 0.2, size=1.2, 
  ...
  theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0),
        axis.title=element_text(size=12,face="bold")) 
ggsave("~/Desktop/GI_quiescence/sum/plot/FigS3A.pdf", width = 7, height = 4.5)
...
ggscatter(TOR1, x = "estimate.single", y = "estimate.double", alpha = 0.5, size=1, 
  ...
  theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), 
        axis.title=element_text(size=12,face="bold")) 
ggsave("~/Desktop/GI_quiescence/sum/plot/Appendix_Figure3B.pdf", width = 7, height = 7)
...
write_csv(PHO85, "~/Desktop/GI_quiescence/sum/GIS analysis/GIS_methods_comp_wtNormed_PHO85.csv")
...
ggscatter(PHO85, x = "estimate.single", y = "estimate.double", alpha = 0.7, size=0.7, 
  ...
  theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), 
        axis.title=element_text(size=12,face="bold")) 
ggsave("~/Desktop/GI_quiescence/sum/plot/Appendix_Figure3C.pdf", width = 7, height = 7)
```
...
GI_VST_all_wtNormed <- GI_VST_all_wtNormed %>%
  mutate(GrowStage = factor(ifelse(GrowStage == "G", "Proliferation", "Quiescence")))
...
GI_wtNormed_sig_pro <- GI_VST_all_wtNormed %>% 
  dplyr::filter(GrowStage == "Proliferation", q.value<0.05) %>%
  dplyr::select(-term)

GI_wtNormed_sig_qui <- GI_VST_all_wtNormed %>% 
  dplyr::filter(GrowStage == "Quiescence", q.value<0.05) %>%
  dplyr::select(-term)

#write significant interacting genes in separate table: positive and negative
write_csv(GI_wtNormed_sig_pro, "~/Desktop/GI_quiescence/sum/GIS analysis/GI_wtNormed_sig_pro.csv")
write_csv(GI_wtNormed_sig_qui, "~/Desktop/GI_quiescence/sum/GIS analysis/GI_wtNormed_sig_qui.csv")
...
ggplot() +
 ...
 # theme(axis.text.x=element_text(angle=90),legend.position="top") +
  scale_fill_manual(values=c("#3399FF", "#FFCC33")) +
  scale_color_manual(values=c("#3399FF", "#FFCC33")) 
ggsave(paste0(path,"Figure3D.pdf"), width =7, height = 8)
...
ggscatter(GI_reshape_pro, x = "carbon_G_PHO85", y = "carbon_G_RIM15", size=1.5, alpha = 0.5,  cor.coef.size = 0.3, col="#FF99A6") +
    xlab("GI of PHO85") +
    ylab("GI of RIM15") +
...
ggsave(paste0(path,"PHO85_RIM15_C_pro.pdf"), width = 4, height = 3.5)

ggscatter(GI_reshape_pro, x = "phosphate_G_PHO85", y = "phosphate_G_RIM15",size=1.5, alpha = 0.5, cor.coef.size = 0.3, col="#FF99A6") +
    xlab("GI of PHO85") +
    ylab("GI of RIM15") +
    geom_smooth(method = "lm", se = FALSE, col = "#F76F87") +
    xlim(-0.5,0.5) +
  ylim(-0.5,0.5) +
    stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top", size=5, col = "#F76F87") +
    theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), axis.title=element_text(size=12,face="bold"))
ggsave(paste0(path,"PHO85_RIM15_P_pro.pdf"), width = 4, height = 3.5)

#Figure S4E, TOR1 and PHO85 carbon proliferation and quiescence. 
ggscatter(GI_reshape_pro, x = "carbon_G_TOR1", y = "nitrogen_G_TOR1",size=1.5, alpha = 0.5, cor.coef.size = 0.3, col="#9999FF") +
    ...
    theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), axis.title=element_text(size=12,face="bold")) 
ggsave(paste0(path,"TOR1_C_N_pro.pdf"), width = 4, height = 3.5)

ggscatter(GI_reshape_qui, x = "carbon_S_TOR1", y = "nitrogen_S_TOR1", size=1.5, alpha = 0.5, cor.coef.size = 0.3, col="#FF99A6") +
    xlab("GI of TOR1 (carbon limited)") +
    ylab("GI of TOR1 (nitrogen limited)") +
    geom_smooth(method = "lm", se = FALSE, col = "#F76F87") +
    xlim(-0.5,0.5)+
    ...
    stat_cor(method = "pearson", label.x.npc = "left", label.y.npc = "top", size=5, col = "#F76F87") +
    theme(axis.text=element_text(size=12), axis.text.x=element_text(angle=0), axis.title=element_text(size=12,face="bold")) 
ggsave(paste0(path,"TOR1_C_N_qui.pdf"), width = 4, height = 3.5)
```

####Condition-dependend genetic interactions mapped by ANCOVA
```{r, include=F}
#YtoGene <- read.table("~/Desktop/GI_quiescence/DESeq2 normalized data/YORFtoGENE_2018")
#colnames(YtoGene) <- c("Strain","Gene")
...
write.csv(master_GI_pro, "~/Desktop/GI_quiescence/sum/GIS analysis/GIs_cytoscape/anova_GI_VST_pro.csv", row.names = FALSE)
...
#pro_cor<-rcorr(as.matrix(master_GI_pro)[,-1])
#pro_cor<-flattenCorrMatrix(pro_cor$r, pro_cor$P) 
...
master_GI_qui <- master_GI_df %>% 
  ...
  dplyr::select (cond, Strain, estimate) %>%
  spread (key = cond, value = estimate)
write.csv(master_GI_qui, "~/Desktop/GI_quiescence/sum/GIS analysis/GIs_cytoscape/anova_GI_VST_qui.csv", row.names = FALSE)
...
```{r}
path = "~/Desktop/GI_quiescence/sum/safe/"
GI_VST_all_wtNormed<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/anova_GI_VST_stage_wtNormed(lm).csv")[,-1]

GI_VST_all <- left_join(GI_VST_all_wtNormed, YtoGene, by ="Strain")

#replace G by proliferation and S by quiescence
GI_VST_all <- GI_VST_all %>% 
  mutate(GrowStage = factor(ifelse(GrowStage == "G", "proliferation", "quiescence")))
...
##Functional annotation of significantly interacting genes
...
pos_int <- GI_VST_all %>% filter (estimate > 0 & q.value < 0.05) %>%
  unite(condition, nutrient, GrowStage) %>%
  dplyr::select(condition, Strain, LibraryName, Common, estimate) %>%
...
TopGIS_Entrez_carbon_s<- pos_int %>% dplyr::filter(LibraryName == var)  %>%
          dplyr::select(Systematic,carbon_S) %>%
          #remove NA values
          drop_na() %>%
  left_join(SystematicToSGDIDmapper, by="Systematic")%>%
  {clusterProfiler::bitr(.$SGDID, fromType="SGD", toType="ENTREZID"
...
TopGIS_Entrez_nitrogen_s<- pos_int %>% dplyr::filter(LibraryName == var)  %>%
          dplyr::select(Systematic,nitrogen_S) %>%
          #remove NA values
          drop_na() %>%
  left_join(SystematicToSGDIDmapper, by="Systematic")%>%
  {clusterProfiler::bitr(.$SGDID, fromType="SGD", toType="ENTREZID"
...
TopGIS_Entrez_phosphate_s <- pos_int %>% dplyr::filter(LibraryName == var)  %>%
          dplyr::select(Systematic,phosphate_S) %>%
          #remove NA values
          drop_na() %>%
  left_join(SystematicToSGDIDmapper, by="Systematic")%>%
  {clusterProfiler::bitr(.$SGDID, fromType="SGD", toType="ENTREZID"
...
dotplot(formula_res_all) + 
  ...
  ggplot2::ggtitle(var)
ggsave(paste0(path,var,"/formula_res_pos.pdf"), width=10,height=8)

formula_res_all[,] %>% 
  write_csv(paste0("~/Desktop/GI_quiescence/sum/GIS analysis/",var,"/formula_res_all_pos.csv"))

formula_res_all_neg[,] %>% 
  write_csv(paste0("~/Desktop/GI_quiescence/sum/GIS analysis/",var,"/formula_res_all_neg.csv"))
}
...
all_pos<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/tor1/formula_res_all_pos.csv")
all_neg<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/tor1/formula_res_all_neg.csv")
...
formula_res_pos_min <- read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/tor1/formula_res_all_pos_min.csv")
...
formula_res_neg_min <- read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/tor1/formula_res_all_neg_min.csv")
...
all_pos<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/rim15/formula_res_all_pos.csv")
all_neg<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/rim15/formula_res_all_neg.csv")

#remove the redundant terms for each condition manually
formula_res_pos_min <- read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/rim15/formula_res_all_pos_min.csv")
...
formula_res_pos_min %>%
  ...
  #theme(text = element_text(size=45),axis.text.x=element_text(angle=90),legend.position="right") +
  #theme_minimal()
ggsave(paste0(path, "Figure6A_up.pdf"), width = 10, height = 5.5)

#remove duplicated terms represented by the same gene group
formula_res_all_neg_min <- read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/rim15/formula_res_all_neg_min.csv")
...
formula_res_all_neg_min %>%
  ...
  theme(axis.text = element_text(color = "black", size = 12),
        ...
        panel.grid.major = element_line(colour="lightgrey", size = 0.3),
        panel.border = element_rect(colour = "black", fill=NA, size=5)) 
ggsave(paste0(path, "Figure6A_dn.pdf"))
```
...
GI_VST_all_wtNormed<-read_csv("~/Desktop/GI_quiescence/sum/GIS analysis/anova_GI_VST_stage_wtNormed(lm).csv") %>%
  dplyr::rename(Systematic = Strain)
...
pdf(paste0(path,"heatmap_ERAD_GI.pdf"), width = 10, height = 5)
...
#  i="ALL"
  ...
  gseGOtable <- bind_rows(gseGOResults) %>% as_tibble %>% 
    ...
    # Then we pick the variables we want
    arrange(Variable,-enrichmentScore)
  gseGOtable %>% write_csv(path=paste0("~/Desktop/GI_quiescence/sum/GIS analysis/output_table_gseGO_GI_",i,".csv"), append=T, col_names = T)
}
```